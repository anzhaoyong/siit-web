...
public class JeecgOneToMainUtil {
	...
	public static void main(String[] args) {
		...
		mainTable.setTableName("siit_train");//表名
		mainTable.setEntityName("Train");	 //实体名
		mainTable.setEntityPackage("siit");	 //包名
		mainTable.setFtlDescription("培训管理");	 //描述
		
		//第二步：设置子表集合配置
		List<SubTableVo> subTables = new ArrayList<SubTableVo>();
		//[1].子表一
		SubTableVo po = new SubTableVo();
		po.setTableName("siit_train_person");//表名
		po.setEntityName("TrainPerson");	    //实体名
		po.setEntityPackage("siit");	        //包名
		po.setFtlDescription("培训报名人员明细");       //描述
		//子表外键参数配置
		/*说明: 
		 ...
		 * c) 多个外键字段，采用逗号分隔;
		*/
		po.setForeignKeys(new String[]{"train_id"});
		subTables.add(po);
		//[2].子表二
